...
"""
        ...
        try:
            ...
            if self.cache_file.exists():
                try:
                    ...
                    print_status(f"Old dependency cache removed: {self.cache_file}")
                except Exception as e:
            ...
            # 4. Save iptables rules and create restore service
    ...
    def _check_network_status(self, username: str) -> bool:
        """Check if network restrictions are active for a user (robust: OUTPUT jump + custom chain REJECT/DROP)."""
        try:
            # Get UID for the username
            try:
                uid = pwd.getpwnam(username).pw_uid
            except Exception:
                print_error(f"Could not get UID for user '{username}'")
                return False
            print_status(f"[DEBUG] UID for user '{username}': {uid}")

            # 1. Find jump to custom chain in OUTPUT for this UID
            result = subprocess.run(
                ['iptables', '-t', 'filter', '-L', 'OUTPUT', '-n', '--line-numbers'],
                capture_output=True, text=True
            )
            if result.returncode != 0:
                print_error("Could not get iptables rules")
                return False
            lines = result.stdout.splitlines()
            custom_chain = None
            for line in lines:
                if f"owner UID match {uid}" in line and "CONTEST_" in line:
                    # Extract chain name (e.g., CONTEST_TESTUSER_OUT)
                    parts = line.split()
                    if len(parts) > 1:
                        custom_chain = parts[1]
                        print_status(f"[DEBUG] Found OUTPUT jump to custom chain: {custom_chain}")
                        break
            if not custom_chain:
                print_status("[DEBUG] No OUTPUT jump to custom chain found for UID")
                return False

            # 2. Check custom chain for REJECT or DROP at the end
            result_chain = subprocess.run(
                ['iptables', '-t', 'filter', '-L', custom_chain, '-n', '--line-numbers'],
                capture_output=True, text=True
            )
            if result_chain.returncode != 0:
                print_error(f"Could not get iptables rules for chain {custom_chain}")
                return False
            chain_lines = [l for l in result_chain.stdout.splitlines() if l.strip() and not l.startswith('Chain') and not l.startswith('num')]
            if not chain_lines:
                print_status(f"[DEBUG] Custom chain {custom_chain} is empty")
                return False
            last_rule = chain_lines[-1]
            print_status(f"[DEBUG] Last rule in {custom_chain}: {last_rule}")
            if "REJECT" in last_rule or "DROP" in last_rule:
                print_status(f"[DEBUG] Custom chain ends with REJECT/DROP")
                return True
            print_status(f"[DEBUG] Custom chain does not end with REJECT/DROP")
            return False
        except Exception as e:
            print_error(f"Failed to check network status: {e}")
            return False
    
    def _check_usb_status(self, username: str) -> bool:
        """Check if USB restrictions are active for a user (udev and polkit rules present)."""
        try:
            from ..utils.usb_restrictor import USBRestrictor
            restrictor = USBRestrictor(username)
            udev_active = os.path.exists(restrictor.udev_rules_path)
            polkit_active = os.path.exists(restrictor.polkit_rules_path)
            # Optionally, check if rules contain block logic
            if not udev_active and not polkit_active:
                print_status("[DEBUG] No USB restriction rules found.")
                return False
            # Optionally, parse rules for block lines
            if udev_active:
                try:
                    with open(restrictor.udev_rules_path, 'r') as f:
                        content = f.read()
                        if 'usb-storage' in content or 'bInterfaceClass' in content:
                            print_status("[DEBUG] udev rules contain block logic.")
                            return True
                except Exception as e:
                    print_status(f"[DEBUG] Could not read udev rules: {e}")
            if polkit_active:
                try:
                    with open(restrictor.polkit_rules_path, 'r') as f:
                        content = f.read()
                        if 'usb' in content and 'block' in content:
                            print_status("[DEBUG] polkit rules contain block logic.")
                            return True
                except Exception as e:
                    print_status(f"[DEBUG] Could not read polkit rules: {e}")
            print_status("[DEBUG] USB restriction rules present but no block logic found.")
            return udev_active or polkit_active
        except Exception as e:
            print_error(f"Failed to check USB status: {e}")
            return False
