...
# --- Module-level API for manager.py and CLI ---
def apply_network_restrictions(user: str, whitelist_file: str) -> bool:
    """Apply network restrictions for a user using the given whitelist file."""
    try:
        ...
        restrictor = NetworkRestrictor(user)
        # No dependencies at this stage; only allowlist
        restrictor.apply_restrictions(domains, set())
        return True
    except Exception as e:
        print(f"[ERROR] Failed to apply network restrictions: {e}")
        return False

def remove_network_restrictions(user: str) -> bool:
    """Remove all network restrictions for a user."""
    try:
        restrictor = NetworkRestrictor(user)
        return restrictor.remove_restrictions()
    except Exception as e:
        print(f"[ERROR] Failed to remove network restrictions: {e}")
        return False
